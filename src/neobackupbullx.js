    ...
    chrome.storage.local
      ...
      .then((result) => {
        ...
        const migrationElement = document.querySelector(
          elementsClassname?.placement_selectors?.neobackupbullx?.token?.snipe
            ?.migrating,
        );
        ...
        if (migrationElement) {
          const migrationText = Array.from(
            migrationElement.querySelectorAll(
              elementsClassname?.placement_selectors?.neobackupbullx?.token
                ?.snipe?.element,
            ),
          ).filter((p) =>
  ...
  try {
    ...
    chrome.storage.local
      ...
      .then((result) => {
        ...
        let isBuy = result.is_buy_neobackupbullx;
      ...
      .then((result) => {
        ...
        const cards = Array.from(
          document.querySelectorAll(
            elementsClassname?.placement_selectors?.neobackupbullx?.memescope
              ?.card,
          ),
        );
        ...
        cards.forEach((card) => {
          ...
          if (isMigrating) {
            let actionSpan = Array.from(
              card.querySelectorAll(
                elementsClassname?.placement_selectors?.neobackupbullx
                  ?.memescope?.migrating?.element,
              ),
            ).find(
              (span) =>
                span.textContent ===
                elementsClassname?.placement_selectors?.neobackupbullx
                  ?.memescope?.migrating?.content,
            );
...
chrome.runtime.onMessage.addListener(async function (request) {
  chrome.storage.local
    ...
    .then(async (result) => {
      ...
      if (request.message === "neo-backup-bullx-pump-vision") {
      ...
      if (request.message === "neo-backup-bullx-token-save") {
      ...
      if (request.message === "neo-backup-bullx-token") {
